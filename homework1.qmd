...
q2 <- flights %>%
  ...
  # need to learn a better way to return a repeat variable
  group_by(month) %>%
  summarise(prop_cancelled = mean(prop_cancelled)) %>%
  arrange(prop_cancelled)

  view(q2)

  # I wish I could figure out how to return just the highest and lowest values rather than the df but the answer is lowest proportion in October and highest in February. The fall (August - November) seems to be the least likely for cancellation but I don't see much seasonality otherwise.
...
q5.2 <- planes %>%
  # n = number of planes from each manufacturer
  add_count(manufacturer) %>% 
  # really need to find a better way to return a repeat variable
...
q6.1 <- flights %>%
  left_join(planes, by = join_by(tailnum))%>% # join the data with plane ages
  slice(which.min(year.y)) # select only the oldest 
                           # (noting that there are two 'year' variables now)
# tailnum = N381AA

q6.2 <- inner_join(flights,planes,by = join_by(tailnum)) %>% # inner join to find shared 
  group_by(tailnum) %>% # find unique ids
  summarise() %>% # rm unnecessary columns
...
q7 <- flights %>%
  group_by(month) %>%
  summarise(med_arr_delay = median(!is.na(arr_delay)))
  
ggplot(flights, aes(x = as.factor(month), y = arr_delay, color = origin)) +
  geom_boxplot() +
  facet_wrap(~as.factor(month), scale = "free")
...
halfrule <- age_gaps %>%
  ...
  ggplot(aes(x = character_1_gender, y = actor_1_age, color = acceptable)) +
  geom_boxplot()
halfrule
# 
```
...
>
> *Not entirely, but that's true of every piece of code I've written. But for the most part, I do believe I can replicate it.*
