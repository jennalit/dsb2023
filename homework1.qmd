...
fly_into_sfo <- flights %>%
  left_join(airlines, by = join_by(carrier)) %>% # get carrier names
  add_count(carrier) %>% # n = total flights by that carrier
  filter(dest == "SFO") %>%
  add_count(carrier) %>% # nn = SFO flights
  mutate(percent_sfo = nn/n *100) %>%
  group_by(name) %>%
  summarise(count = mean(nn),percent = mean(percent_sfo)) # I don't know how to group by multiple variables, this was easier
